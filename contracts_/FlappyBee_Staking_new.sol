...
contract StakingBEET {
    ...
    function claimReward() external {
        ...
        for (uint256 i = 0; i < stakes[msg.sender].length; i++) {
            ...
            stakes[msg.sender][i].lastClaimTime = block.timestamp;
        }

        require(token.transfer(msg.sender, totalReward), "Claim rewards failed!");
    ...
    function calculateReward(address user, uint256 index) public view returns (uint256) {
        ...
        if(userStake.lastClaimTime == 0){
            stakeDuration   = block.timestamp - userStake.startTime;
        }
        else{
            stakeDuration = block.timestamp - userStake.lastClaimTime;
        }
    ...
    function getRemainedLockTime(address user, uint256 index) external view returns (uint256) {
        require(stakes[user].length > index, "Invalid index");

        StakedToken storage userStake = stakes[user][index];
        require(userStake.active, "No active stake");

        for (uint256 i = 0; i < unstakedInfo[user].length; i++) {
            if (unstakedInfo[user][i].amount == userStake.amount && !unstakedInfo[user][i].withdrawn) {
                if (block.timestamp < unstakedInfo[user][i].unlockTime) {
                    return unstakedInfo[user][i].unlockTime - block.timestamp;
                } else {
                    return 0;
                }
            }
        ...
        return 0; // Return 0 if no corresponding unstaked token is found
    }
